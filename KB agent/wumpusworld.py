...
class WumpusWorldAgent(inference.KnowledgeBasedAgent):
  ...
  def safe(self):
    ...
    for x in range(1, self.size+1):
      for y in range(1, self.size+1):
        not_pit = logic.expr('~P%d_%d' % (x, y))
        not_wumpus = logic.expr('~W%d_%d' % (x, y))
        no_breeze = logic.expr('~B%d_%d' % (x, y))
        no_stench = logic.expr('~S%d_%d' % (x, y))
        if inference.resolution(self.KB, not_pit) and inference.resolution(self.KB, not_wumpus):
          safe.add((x, y))
        if inference.resolution(self.KB, no_breeze) and inference.resolution(self.KB, no_stench):
  ...
  """
  Create a set of maybe unsafe spaces, and, using two for loops, go through every space.
  ...
  def maybe_unsafe(self):
    ...
    for x in range(1, self.size+1):
      for y in range(1, self.size+1):
        pit = logic.expr('P%d_%d' % (x, y))
        wumpus = logic.expr('W%d_%d' % (x, y))
        breeze = logic.expr('B%d_%d' % (x, y))
        stench = logic.expr('S%d_%d' % (x, y))
        if inference.resolution(self.KB, pit) or inference.resolution(self.KB, wumpus):
          maybe_unsafe.add((x, y))
        if inference.resolution(self.KB, breeze) or inference.resolution(self.KB, stench):
          for z in get_neighbors(x, y, self.size):
            maybe_unsafe.add(z)

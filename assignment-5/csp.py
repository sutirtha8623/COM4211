...
def min_conflicts(csp, max_steps=10000):
    ...
    for _ in range(max_steps):
        if len(csp.conflicted_vars(curr)) == 0:
            return curr
        # csp.display(curr)
        # print()
        var = random.choice(csp.conflicted_vars(curr))
        # csp.unassign(var, curr)
        value = min_conflicts_value(csp, var, curr)

        csp.assign(var, value, curr)
    return None
